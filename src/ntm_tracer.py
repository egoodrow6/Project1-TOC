...
class NTM_Tracer(TuringMachineSimulator):
    def run(self, input_string, max_depth):
        ...
        while depth < max_depth and not accepted:
            ...
            # Print the current level and configurations
            ...
            for config in current_level:
                ...
                # This works for the root (len 3) AND children (len 4)
                ...
                # A. Retrieve all rules for the current state (default to empty list if state not found)
                state_rules = self.transitions.get(state, [])

                # B. Iterate through rules to find matches for the current symbol
                for rule in state_rules:
                    # The debug output shows 'read' is a tuple like ('a',), so we access index 0
                    read_symbol = rule['read'][0]
                    
                    if read_symbol == symbol:
                        # Extract details from the dictionary structure shown in your logs
                        next_st = rule['next']        # This is a string 'q1'
                        write_ch = rule['write'][0]   # This is a tuple ('a',), so take [0]
                        dir_mv = rule['move'][0]      # This is a tuple ('R',), so take [0]
                        
                        # Add to possible moves as a simple tuple
                        possible_moves.append((next_st, write_ch, dir_mv))

                # 5. If no explicit transition exists, treat as implicit Reject
                if not possible_moves:
                    # No transitions defined for (state, symbol).
                    # The branch ends here (implicit reject).
                    #print("No valid moves, rejecting this branch")
                    continue
                
                # If we are here, we have valid moves, so the tree is still alive
                all_rejected = False
                ...
                for move in possible_moves:
                    ...
                    # Remember: 'right' includes the character currently under the head at index 0
                    ...
                    if direction == 'R':
                        ...
                        if len(right) > 0:
                            ...
                            # If we moved right into a blank area, the new head sees empty (blank)
                            new_right = ""
                        ...
                        if len(left) > 0:
                            ...
                            # Standard behavior: The head stays put, but the write still happens.
                            rest_of_right = right[1:] if len(right) > 0 else ""
                            new_right = write_char + rest_of_right
                            new_left = ""

                    # Create the new configuration and add to the next level
                    # NEW (Tuple recommended so it's immutable, or list is fine):
                    # We append the 'config' variable (the parent) as the 4th element
                    new_config = [new_left, next_state, new_right, config]
                    next_level.append(new_config)

                    # 7. Prepare for next iteration
            
            # If no valid moves were found for ANY configuration in this level,
            # the machine halts and rejects.
            if all_rejected:
    ...
    def print_trace_path(self, final_node):
        ...
        # 1. Backtrack 
        # We check if current has 4 elements (meaning it has a parent)
